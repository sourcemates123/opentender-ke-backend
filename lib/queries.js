...
const Aggregations = {
		...
		request: {
			...
			'aggregations': {
				'authority_nuts_nested': {
					'terms': {
						'field': 'buyers.address.ot.nutscode',
						'size': MAX_NUTS_AGGREGATION
					}
				},
				'authority_nuts_nested_contracts': {
					'terms': {
	...
	histogram_finalPriceEUR: {
		...
		parse: (answer, options) => {
			...
			buckets.forEach(bucket => {
				let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
				bucket.sum_finalPriceEUR.value = Utils.roundValueTwoDecimals(bucket.sum_finalPriceEUR.value);
				bucket.avg_finalPriceEUR.value = Utils.roundValueTwoDecimals(bucket.avg_finalPriceEUR.value);
				result[year] = {value: bucket.doc_count, sum_finalPriceEUR: bucket.sum_finalPriceEUR, avg_finalPriceEUR: bucket.avg_finalPriceEUR};
			});
			return result;
		}
	},
