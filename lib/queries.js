...
const Aggregations = {
	...
	histogram_finalPriceEUR: {
			...
			'aggregations': {
				'finalPriceEUR': {
					...
					'aggregations': {
						'sum_finalPriceEUR': {
							'sum': {
								'field': 'lots.estimatedPrice.netAmountNational',
							},
						},
						'avg_finalPriceEUR': {
							'avg': {
		...
		parse: (answer, options) => {
			...
			buckets.forEach(bucket => {
				let year = parseInt(bucket.key_as_string.slice(0, 4), 10);
				bucket.finalPriceEUR.sum_finalPriceEUR.value = Utils.roundValueTwoDecimals(bucket.finalPriceEUR.sum_finalPriceEUR.value);
				bucket.finalPriceEUR.avg_finalPriceEUR.value = Utils.roundValueTwoDecimals(bucket.finalPriceEUR.avg_finalPriceEUR.value);
				result[year] = {
					value: bucket.doc_count,
					sum_finalPriceEUR: bucket.finalPriceEUR.sum_finalPriceEUR,
					avg_finalPriceEUR: bucket.finalPriceEUR.avg_finalPriceEUR,
				};
			});
			return result;
		},
