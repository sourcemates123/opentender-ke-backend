...
class Converter {
	...
	cleanIndicators(list) {
		...
		const indicatorsToSkip = [
			'TRANSPARENCY_NUMBER_OF_KEY_MISSING_FIELDS',
			'INTEGRITY_NEW_COMPANY',
			'INTEGRITY_TAX_HAVEN',
		]
		let filteredList = list.filter((indicator) => !indicatorsToSkip.includes(indicator.type))
		if (filteredList) {
			filteredList.forEach(doc => {
				if (doc.type === 'TRANSPARENCY_NOTICE_AND_AWARD_DISCREPANCIES') {
					doc.type = 'ADMINISTRATIVE_NOTICE_AND_AWARD_DISCREPANCIES';
				}
				if (!isNaN(doc.value)) {
	...
	cleanLots(list) {
		...
		if (list) {
			list.forEach((v) => {
				if (v.endorsementDate === '') { delete v.endorsementDate; }
				if (v.cabinetApprovalDate === '') { delete v.cabinetApprovalDate; }
				if (v.committeeApprovalDate === '' || v.committeeApprovalDate === 'Y' || v.committeeApprovalDate === 'N') { delete v.committeeApprovalDate; }
				if (v.headOfEntityApprovalDate === '') { delete v.headOfEntityApprovalDate; }
			});
	...
	cleanFinalPrice(lots) {
		let finalPrice = {
			"netAmount": 0,
			"currency": "UGX",
			"currencyNational": "UGX",
			"netAmountNational": 0
		}
		...
		lots.forEach(lot => {
			if (lot.bids) {
				lot.bids.forEach(bid => {
					finalPrice.netAmount += (bid.price.netAmount || 0)
					finalPrice.netAmountNational += (bid.price.netAmountNational || 0)
				})
			}
		});
	...
	cleanBody(doc) {
		...
		this.cleanProperties(['groupId', 'created', 'modified', 'createdBy', 'createdByVersion', 'modifiedBy', 'modifiedByVersion', '_id', 'bodyIds', 'metaData'], doc);
		if (this.stats) {
	...
	chooseTenderDate(doc) {
		let dates = [doc.awardDecisionDate];
		if (doc.lots) {
		...
		if (doc.publications) {
			doc.publications.forEach(publication => {
	...
	cleanCommitteeApprovalDate(committeeApprovalDate) {
		if (committeeApprovalDate) {
			committeeApprovalDate = committeeApprovalDate.slice(0, 4)
		}
	...
	cleanTenders(items) {
		return items.map(item => {
			this.cleanProperties(['created', 'modified', 'persistentId', 'processingOrder', 'indicators'], item);
			return item;
		});
	}
	transform(items) {
