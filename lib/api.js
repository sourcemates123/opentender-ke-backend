        ...
        this.store.Tender.get(id, (err, result) => {
            if (err || !result) {
                ...
                if (result.hits.total < 1) {
                    ...
                    tender.supplierId = suppliers.map(supplier => supplier.id);
                    tender.suppllierName = suppliers.map(supplier => supplier.name);
                    tender.supplierAddress = suppliers.map(supplier => supplier.address.rawAddress);
                    tender.supplierBidderType = suppliers.map(supplier => supplier.bidderType);
