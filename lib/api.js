...
class Api {
  ...
  getIndicatorScoreStats(options, country_id, cb) {
    ...
    this.store.Tender.search(b, 0, 0, (err, result) => {
      ...
      let children = [];
      if (result.aggregations.terms_additional) {
        ...
        const newAggs = Queries.buildAggregations([
          'terms_main_cpv_divisions',
          ...
          'histogram',
					'map_chart_indicators_count',
					'map_chart_indicators_value'
        ]);
        ...
        let nuts_stats_aggs = Queries.buildAggregations(['sum_finalPriceEUR', 'avg_finalPriceEUR']);
				aggs.request['map_chart_indicators_count'].aggregations = newAggs.request['map_chart_indicators_count'];
				aggs.request['map_chart_indicators_value'].aggregations = newAggs.request['map_chart_indicators_value'];
      }
      let stats = {};
      ...
      stats.map_chart_indicators = {
        contracts: {
          count: data.map_chart_indicators_count || 0,
          value: data.map_chart_indicators_value || 0,
        },
      };
      cb(null, stats);
    });
  ...
  getIndicatorRangeStats(options, country_id, cb) {
    ...
    this.store.Tender.search(b, 0, 0, (err, b) => {
      if (err) {
        console.error(err);
        return cb(err);
      }
      let stats = aggs.parse(b.aggregations, { library: this.library, lang: options.lang });
			if (b.aggregations.terms_additional) {
				let buckets = b.aggregations.terms_subregions.buyers_filter.nuts.buckets;
				buckets.forEach(bucket => {
					let code = bucket.key.toUpperCase();
					if (code !== nutsInfo.nuts.id) {
						stats.terms_subregions_nuts[code] = bucket.tenders.doc_count;
    ...
    this.store.Tender.get(id, (err, result) => {
      if (err || !result) {
        ...
        if (result.hits.total < 1) {
          ...
          if (tender.lots && tender.lots[0] && tender.lots[0].bids && tender.lots[0].bids[0] && tender.lots[0].bids[0].bidders && tender.lots[0].bids[0].bidders[0]) {
            const [supplier] = tender.lots[0].bids[0].bidders;
            tender.supplierId = supplier.id;
            tender.suppllierName = supplier.name;
            tender.supplierAddress = supplier.address.rawAddress;
            tender.supplierBidderType = supplier.bidderType;
          }
          console.log(tender);
  ...
  searchTender(options, country_id, cb) {
    ...
    this.store.Tender.search(body, (options.size || 10), (options.from || 0), (err, result) => {
      ...
      let stats = undefined;
      if (aggs) {
        stats = aggs.parse(result.aggregations, { library: this.library, lang: options.lang });
      }
      let aggregations = {};
      ...
      if (result.aggregations) {
        Object.keys(result.aggregations).forEach(key => {
          if (key[0] === '_') {
            aggregations[key] = result.aggregations[key];
          }
        });
      }
      let answer = {
